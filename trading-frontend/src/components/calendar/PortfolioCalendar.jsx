...
const PortfolioCalendar = ({
  ...
  // Used for calendar tile color/value.
  // This should remain normalized so deposits/withdrawals do not create fake PnL.
  const curve = charts?.[curveKey] || [];

  // Used for day modal open/close/PNL/ROI.
  // This contains portfolio_close, actual_value, pre_open_net_flow, after_close_net_flow.
  const actualCurve = charts?.actual_equity_curve || [];
  ...
  const actualDailyDetailsByDate = useMemo(() => {
    if (!Array.isArray(actualCurve) || actualCurve.length === 0) return {};

    const details = {};

    for (let i = 0; i < actualCurve.length; i++) {
      const point = actualCurve[i];
      const iso = String(point.date);

      const previousPoint = i === 0 ? point : actualCurve[i - 1];

      const previousActualValue = safeNumber(
        previousPoint.actual_value ??
          previousPoint.value ??
          previousPoint.portfolio_close
      );

      const preOpenNetFlow = safeNumber(point.pre_open_net_flow ?? 0);

      const portfolioClose = safeNumber(
        point.portfolio_close ?? point.value ?? point.actual_value
      );

      const open = i === 0
        ? portfolioClose
        : previousActualValue + preOpenNetFlow;

      const close = portfolioClose;
      const pnl = close - open;
      const roi = open !== 0 ? (pnl / open) * 100 : 0;
      ...
      details[iso] = {
        ...
        roi,
        preOpenNetFlow,
        afterCloseNetFlow: safeNumber(point.after_close_net_flow ?? 0),
        actualValue: safeNumber(point.actual_value ?? point.value),
        source: "actual",
      };
    }
  ...
  const openDayModal = (date) => {
    ...
    const details =
      actualDailyDetailsByDate[iso] ||
      dailyDetailsByDate[iso];
  ...
  return (
    <div className="calendar-container">
      ...
      >
        {detailModal?.type === "day" && (
          <div className="calendar-detail calendar-detail-daily">
            ...
            <div className="calendar-detail-divider" />
